...
class ReentryTimelineView(QWidget):
    ...
    def __init__(self, db):
        ...
        # Country/region groupings for re-entry zones
        self.country_groups = {
            'China': ['Jiuquan', 'Dong feng', 'LOP NOR', 'Inner Mongolia'],
            'USA': ['Cape Canaveral', 'Vandenberg', 'Pacific Ocean', 'Atlantic Ocean', 'Starbase'],
            'Russia': ['Kazakh Steppe', 'Siberia'],
            'Europe': ['Atlantic', 'North Sea'],
            'Pacific': ['NZ EEZ', 'Pacific Ocean', 'Koonibba'],
            'India': ['Bay of Bengal', 'Indian Ocean'],
        }
        
        self.expanded_groups = set(['China', 'Pacific'])
        self.init_ui()
    ...
    def update_timeline(self):
        ...
        # Build rows grouped by country/region
        rows = []
        for country, locations in self.country_groups.items():
            country_has_reentries = False
            country_zones = []
            
            # Get all re-entry sites
            all_sites = self.db.get_all_sites(site_type='REENTRY')
            
            for location in locations:
                location_zones = [s for s in all_sites if s['location'] == location]
                ...
                for zone in location_zones:
                    ...
                    if has_reentries or not self.show_only_active:
                        ...
                        if has_reentries:
                            country_has_reentries = True
            
            if country_zones and (country_has_reentries or not self.show_only_active):
                rows.append({
                    'type': 'group',
                    'country': country,
